class WizardJS {
    ...
    renderGenerator() {
            ...
            <div class="generator-container">
                <div class="generator-layout">
                    <div class="generator-panel">
                        <h2 class="panel-title">${this.t('generator.configuration')}</h2>

                        <div class="form-group">
                            <label class="form-label">${this.t('generator.wizardTitle')}</label>
                            <input type="text" class="form-input" id="genWizardTitle" 
                                   value="${this.generatorConfig.title}">
                        </div>

                        <div class="generator-actions">
                            <button class="btn btn-primary" onclick="wizardInstance.addStepConfig()">
                                ${this.getSVGIcon('plus')} ${this.t('generator.addStep')}
                            </button>
                            <button class="btn btn-secondary" onclick="wizardInstance.loadExample()">
                    ...
                    <div class="generator-panel">
                        <h2 class="panel-title">${this.t('generator.preview')}</h2>
                        
                        <div class="generator-actions">
                            <button class="btn btn-primary" onclick="wizardInstance.exportConfig()">
                                ${this.getSVGIcon('download')} ${this.t('generator.exportJson')}
                            </button>
                            <button class="btn btn-secondary" onclick="wizardInstance.previewWizard()">
                                ${this.getSVGIcon('play')} ${this.t('generator.previewWizard')}
                            </button>
                            <button class="btn btn-secondary" onclick="wizardInstance.copyJSON()">
        ...
        // Guardar referencia global para los event handlers inline
  ...
  renderModals() {
    return `
        <!-- Modal Bootstrap para Pasos -->
        <div class="modal fade" id="stepConfigModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">${this.t('generator.addStep')}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="stepConfigForm" onsubmit="wizardInstance.saveStepConfig(event)">
                            <div class="mb-3">
                                <label class="form-label">${this.t('generator.stepId')}</label>
                                <input type="text" class="form-control" name="stepId" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">${this.t('generator.stepTitle')}</label>
                                <input type="text" class="form-control" name="stepTitle" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">${this.t('generator.stepDescription')}</label>
                                <textarea class="form-control" name="stepDescription" rows="3"></textarea>
                            </div>
                            ...
                            <div id="conditionFields" style="display: none;" class="mb-3">
                                <div class="card">
                                    <div class="card-body">
                                        <h6 class="card-title">Condiciones del Paso</h6>
                                        <div class="row">
                                            <div class="col-md-4">
                                                <label class="form-label">${this.t('generator.conditionField')}</label>
                                                <input type="text" class="form-control" name="conditionField">
                                            </div>
                                            <div class="col-md-4">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">${this.t('generator.cancel')}</button>
                        <button type="submit" form="stepConfigForm" class="btn btn-primary">${this.t('generator.save')}</button>
                    </div>
                </div>
            </div>
        </div>

        <!-- Modal Bootstrap para Campos -->
        <div class="modal fade" id="fieldConfigModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">${this.t('generator.fieldConfig')}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="fieldConfigForm" onsubmit="wizardInstance.saveFieldConfig(event)">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="fieldType" onchange="wizardInstance.toggleFieldOptions()">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="fieldPlaceholder">
                            </div>
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="fieldRequired" id="fieldRequired">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">${this.t('generator.cancel')}</button>
                        <button type="submit" form="fieldConfigForm" class="btn btn-primary">${this.t('generator.save')}</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        container.innerHTML = this.generatorConfig.steps.map((step, index) => `
            <div class="step-config-item">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <h4>${step.title}</h4>
                        <p style="color: #666; font-size: 14px;">ID: ${step.id} | ${step.fields.length} ${this.t('generator.fields')}</p>
                        ${step.condition ? '<span class="field-type" style="background: #fbbf24;">Condicional</span>' : ''}
                    </div>
    ...
    // Ocultar sección de condiciones
    document.getElementById('hasCondition').checked = false;
    document.getElementById('conditionFields').style.display = 'none';
    
    // Limpiar campos de condición
    form.elements['conditionField'].value = '';
    form.elements['conditionOperator'].value = 'equals';
    form.elements['conditionValue'].value = '';
    
    this.openModal('stepConfigModal');
}
    ...
    // Llenar con datos del paso
    form.elements['stepId'].value = step.id;
    form.elements['stepTitle'].value = step.title;
    form.elements['stepDescription'].value = step.description || '';
    
    // Si tiene condición, mostrar y llenar
    if (step.condition) {
        document.getElementById('hasCondition').checked = true;
        document.getElementById('conditionFields').style.display = 'block';
        form.elements['conditionField'].value = step.condition.field;
        form.elements['conditionOperator'].value = step.condition.operator;
        form.elements['conditionValue'].value = step.condition.value;
    }
    ...
    saveStepConfig(event) {
        event.preventDefault();
        const form = event.target;
        
        const stepData = {
            id: form.elements['stepId'].value,
            title: form.elements['stepTitle'].value,
            description: form.elements['stepDescription'].value,
            fields: this.tempFields
        };
        
        if (document.getElementById('hasCondition').checked) {
            stepData.condition = {
                field: form.elements['conditionField'].value,
                operator: form.elements['conditionOperator'].value,
                value: form.elements['conditionValue'].value
            };
        }
        ...
        this.closeModal('stepConfigModal');
        this.updateGeneratorUI();
    }
    ...
    // Resetear tipo de campo a 'text'
    form.elements['fieldType'].value = 'text';
    ...
    saveFieldConfig(event) {
        ...
        const fieldData = {
            name: form.elements['fieldName'].value,
            label: form.elements['fieldLabel'].value,
            type: form.elements['fieldType'].value,
            placeholder: form.elements['fieldPlaceholder'].value,
            required: form.elements['fieldRequired'].checked
        };
    ...
    toggleFieldOptions() {
    const type = document.querySelector('[name="fieldType"]').value;
    ...
    if (['select', 'checkbox', 'radio'].includes(type)) {
        ...
        // Limpiar opciones cuando no se necesitan
    ...
    const options = [];
    const container = document.getElementById('fieldOptionsList');
    const items = container.querySelectorAll('.row'); // Cambiar de '.option-input-group' a '.row'
    ...
    // Limpiar primero el formulario
    form.reset();
    document.getElementById('fieldOptionsContainer').style.display = 'none';
    document.getElementById('fieldOptionsList').innerHTML = '';
    
    // Llenar con datos del campo
    ...
    form.elements['fieldRequired'].checked = field.required || false;
    
    // Si tiene opciones, mostrar y llenar
    if (field.options && ['select', 'checkbox', 'radio'].includes(field.type)) {
        ...
        field.options.forEach(option => {
            const optionDiv = document.createElement('div');
            optionDiv.className = 'row mb-2'; // Usar clases Bootstrap
            optionDiv.innerHTML = `
                <div class="col-5">
...
closeModal(modalId) {
    ...
    loadExample() {
        this.generatorConfig = {
            ...
            steps: [
                ...
                },
                {
                    ...
                    description: "Información sobre tu empresa",
                    condition: {
                        field: "userType",
                        operator: "equals",
                        value: "business"
        ...
        document.getElementById('genWizardTitle').value = this.generatorConfig.title;
        this.updateGeneratorUI();
    }
    ...
    renderField(field, value) {
        switch (field.type) {
            ...
            case 'password':
            case 'number':
                return `<input type="${field.type}" name="${field.name}" class="form-input" 
            ...
            default:
                return `<input type="${field.type}" name="${field.name}" class="form-input" value="${value}" />`;
        }
    }
        ...
        this.container.innerHTML = `
            <div class="wizard-container">
                <div class="success-message">
                    ...
                    <h2>¡Completado!</h2>
                    <p>El wizard se ha completado exitosamente.</p>
                </div>
            </div>
        `;
    }
